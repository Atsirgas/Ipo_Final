...
var camera,
        ...
        ratio = width / height;
var texture = THREE.ImageUtils.loadTexture('360Img/beach.jpg', new THREE.UVMapping(), function() {
    init();
    animate();
});
function init() {
...
function searchImages() {
    var input = document.getElementById('image-search').value;
    var resultsContainer = document.getElementById('image-results');
    resultsContainer.innerHTML = ''; // Limpiar resultados anteriores

    // Simulación de resultados de búsqueda (reemplazar con API real o lógica de búsqueda)
    var images = ['360Img/beach.jpg', '360Img/spherical_texture.jpg', '360Img/ciutat.jpg'];
    images.forEach(function(image) {
        var imgElement = document.createElement('img');
        imgElement.src = image;
        imgElement.style.width = '100px'; // Tamaño de miniatura
        imgElement.onclick = function() { updateScene(image); };
        resultsContainer.appendChild(imgElement);
    });
}

function updateScene(imagePath) {
    var newTexture = THREE.ImageUtils.loadTexture(imagePath, new THREE.UVMapping(), function() {
        scene.children[0].material.map = newTexture;
        render();
    });
}
